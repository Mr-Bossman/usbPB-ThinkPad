...
static struct source_capability source_caps[10];

static volatile uint8_t num_source_caps = 0;

uint8_t usb_pd_get_source_caps(struct source_capability** caps)
{
	if(caps)
		*caps = source_caps;
	return num_source_caps;
}
...
static void usb_pd_handle_src_cap_msg(uint16_t header, const uint8_t* payload)
{
...
void usb_pd_handle_message(uint16_t header, const uint8_t* payload){
	...
	switch (type) {
		case data_source_capabilities:
			usb_pd_handle_src_cap_msg(header, payload);
		break;
			default:
		break;
		}
...
void usb_pd_request_power(uint16_t voltage, uint16_t max_current)
{
	...
	// Send message
	fusb302_send_message(header, payload);
}
